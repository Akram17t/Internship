...
\begin{scope}[on background layer]
  \node[draw=InkSoft!30, fill=Paper, rounded corners=6pt,
        inner sep=12pt, fit=(fastapi)(rag)(ingest), label={[font=\scriptsize\bfseries, color=InkSoft]above left:Python Backend}] {};
\end{scope}

%% Background: Data layer box
\begin{scope}[on background layer]
  \node[draw=LineBorder, fill=PaperDeep, rounded corners=6pt,
        inner sep=12pt, fit=(pg)(chroma), label={[font=\scriptsize\bfseries, color=Muted]below left:Data Layer}] {};
\end{scope}
...
\begin{scope}[on background layer]
  \node[draw=Red!40, fill=WarningBg, rounded corners=6pt,
        ...
        label={[font=\scriptsize\bfseries, color=Red!70]above right:External Services}] {};
\end{scope}
...
% Route FastAPI->PG to the LEFT of the backend column to avoid crossing Researcher Crew
\draw[arr] (fastapi.west) -- ++(-1.2,0) |- (pg.east);
% Route RAG->ChromaDB downward then left to avoid crossing Preprocessing
\draw[arr] (rag.west) -- ++(-1.8,0) |- (chroma.east);
...
\node[store] (ebs1) at ( 3.5, -4.0) {EBS vol\\app-data};
\node[store] (ebs2) at ( 7.5, -4.0) {EBS vol\\pg-data};
...
\draw[darr] (pg)     -- node[lbl, right]{volume} (ebs2);
\draw[darr] (chroma) -- node[lbl, right]{volume} (ebs1);

\end{tikzpicture}%
}
\end{center}
\begin{center}\small\color{Muted}Deployment aktual pada satu EC2 instance; semua container berjalan dalam satu Docker Compose stack.\end{center}
